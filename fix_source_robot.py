...
target_path = os.path.expanduser("~/ros2_ws/src/stonefish_bluerov2/scenarios/bluerov2.scn")

# Write the file
try:
    with open(target_path, "w") as f:
        f.write(xml_content)
    print(f"SUCCESS: Wrote corrected XML to {target_path}")
    
    # Read back to verify
    with open(target_path, "r") as f:
        content = f.read()
        if '<limits min="-1.0" max="1.0"/>' in content:
            print("VERIFIED: Limits tag is definitely present.")
        else:
            print("WARNING: Wrote file but limits tag is NOT found. Something is wrong.")
